...
const OPERTAOR_VALUE_TRANSFORMER: {
    ...
    [OPERATORS.BETWEEN]: (val: string) => {

    }
};

export const SequelizeQueryStringParser = (req: any, res: any, next: any) => {
    const {
        filter,
        order,
        page,
...
const parseFilter = (filterObject: {
    ...
    if (Object.values(OPERATORS).includes(op) && key !== null) {
        return {
            [key]: {
                [SEQUELIZE_OP_MAP[op]]: OPERTAOR_VALUE_TRANSFORMER[op] ? OPERTAOR_VALUE_TRANSFORMER[op](value) : value
            }
        }
    }
